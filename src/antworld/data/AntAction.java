 ...
 * <br>
 * 
 * Each AntData contain a reference to an AntAction instance. The AntAction
 * class is used by the client to tell the server what action the corresponding
 * ant asks to do. Each nest has a unique name. When a client opens a socket to
 * the server for the first time, it must send a nest name in CommData to
 * request possession of a nest.
 * 
...
public class AntAction implements Serializable
{
  ...
  public enum AntActionType
  {
    ...
    EXIT_NEST, // EXIT_NEST x y (must be underground and x,y must be in home
               // nest area)
    BIRTH, // Client adds new ant to antlist, sets ant type. Server deducts
           // needed food from nest store.
    DIED, //
    STASIS // STASIS
  };

  /**
   * Every AntAction must have a type: MOVE | ATTACK | PICKUP | DROP | HEAL |
   * ENTER_NEST | EXIT_NEST | BIRTH |DIED | STASIS
   */
  public AntActionType type;

  /**
   * One of the 8 possible directions. The AntActionTypes requiring Direction
   * are: MOVE, ATTACK, PICKUP, DROP, HEAL (when executed by a medic ant outside
   * of the nest)
   */
  public Direction direction;

  /**
   * (x,y) specify absolute coordinates in the Ant World grid. These fields are
   * only used by the EXIT_NEST action. An EXIT_NEST action is only valid when
   * the specified coordinates are within a Manhattan Distance of
   * Constants.NEST_RADIUS from the ant's own nest center and, of course, when
   * the ant is underground.
   */
  public int x, y;
  ...
  /**
   * Constructor used for an AntActionType that requires ant world map
   * coordinates.
   */
  ...
  public String toString()
  {
    ...
    if (type == AntActionType.MOVE)
      out += direction + "]";
    else if (type == AntActionType.ATTACK)
      out += direction + "]";
    else if (type == AntActionType.PICKUP)
      out += direction + " quentity=" + quantity + "]";
    else if (type == AntActionType.DROP)
      out += direction + " quentity=" + quantity + "]";
    else if (type == AntActionType.HEAL)
      out += direction + "]";
    else if (type == AntActionType.ENTER_NEST)
      out += "]";
    else if (type == AntActionType.EXIT_NEST)
      out += "(" + x + ", " + y + ")]";
    else if (type == AntActionType.STASIS)
      out += "]";

...
/**
 * The Class CommData. !!!!!!!!!! DO NOT MODIFY ANYTHING IN THIS CLASS
 * !!!!!!!!!!<br>
...
public class CommData implements Serializable
{
  ...
  /** The password. */
  public volatile long password; // Must be assigned number when requesting
                                 // nest. Must be zero otherwise

  /** The wall clock milli sec. */
  public volatile long wallClockMilliSec; // latency debugging

  /** The game tick. */
  public volatile int gameTick; // simtime - if received by serv and past this
                                // value, data dropped
   ...
   * The server returns this list with the current ants in this team and their
   * actions.<br>
   * The client uses this list to tell the server which ants are going to do
   * what.<br>
   * The client also adds ants to this list to brith an ant form nest resources.<br>
   * The client returns myAntList to the server with the ants ordered. The ant's
   * actions are executed from first element to last.<br>
   * <br>
   * 
   */
  ...
  // CommData to server
  /**
   * The nest data. To get the location of your own nest, use: <br>
  ...
  /** The enemy ant set. */
  public volatile HashSet<AntData> enemyAntSet; // set to null before sending to
                                                // server.

  /** The food set. */
  public volatile HashSet<FoodData> foodSet; // set to null before sending to
                                             // server.
  ...
  /**
   * Package for socket send to server. When you construct an ObjectOutputStream
   * and an ObjectInputStream, they each contain a cache of objects that have
   * already been sent across this stream. The cache relies on object identity,
   * rather than the traditional hashing function. It is more similar to a
   * java.util.IdentityHashMap than a normal java.util.HashMap. So, if you
   * resend the same object, only a pointer to the object is sent across the
   * network. This is very clever, and saves network bandwidth. However, the
   * ObjectOutputStream cannot detect whether your object was changed
   * internally, resulting in the Receiver just seeing the same object over and
   * over again. You will notice that this was quite fast. We sent 1'500'000
   * objects in 19548ms (on my machine). (well, we only sent one object, and
   * 1'499'999 pointers to that object).
   * 
   *
  ...
  {
    String out = "CommData[" + serialVersionUID + ":" + gameTick + ":"
        + wallClockMilliSec + "]: " + myNest + ", myTeam=" + myTeam + "\n     ";
    if (errorMsg != null)
    {

 ...
 * prevent the server from being able to read this class.<br>
 * <br>
 * 
 * 
 * AntData contains all data about an ant agent that is exchanged between client
 * and server.
 * 
 */
...
public class AntData implements Comparable<AntData>, Serializable
{
   ...
   * client should leave this set to the default value of
   * Constants.UNKNOWN_ANT_ID.<br>
   * <br>
  ...
  /**
   * World Map pixel coordinates of this ant with (0,0) being upper-left. In the
   * ant world map, each game object (an ant or a food pile) occupies exactly
   * one pixel. No two game objects may occupy the same pixel at the same time.
  ...
  /**
   * The number of food units the ant is carrying. An ant may only carry one
   * type of food at a time. The maximum number of units an ant can carry is
   * given by this.antType.getCarryCapacity()
   * */
  public int carryUnits = 0;

  /**
   * The client sets myAction to tell the server what the ant wants to do. The
   * server returns this set to the action that was actually taken.<br>
   * <br>
   * 
   * For example, the server set myAction to STASIS under two conditions:<br>
   * <ol>
   * <li>This ant attempts a move that is not legal (move onto water, attack in
   * a direction where there is not an ant, ...)</li>
  ...
  /**
   * The ticks until next action. This is used for actions that take more than
   * one tick to complete. Currently, the only such action is movement,
   * */
  ...
  /**
   * True when the ant is underground. When an ant is underground, it is in its
   * nest. This means:
   * <ol>
   * <li>The ant can exit its nest any coordinates within its nest radius that
   * is not occupied by a game object.</li>
   * <li>The ant is totally save: it cannot be attacked and never takes
   * attrition damage.</li>
   * <li>The ant may heal itself this.antType.getHealPointsPerWaterUnit() health
   * point by consuming one water drop from the nest storage.</li>
   ...
   *
   * @param id
   *          the id (when birthed by the client, this must be
   *          Constants.UNKNOWN_ANT_ID).
   * @param type
   *          the type
   * @param nestName
   *          the nest name
   ...
   *          the team name
   */
  public AntData(int id, AntType type, NestNameEnum nestName,
      TeamNameEnum teamName)
  {
    this.id = id;
    antType = type;
  ...
  /**
   * Instantiates a new ant data by creating a deep copy of the given source
   * ant.
   *
   * @param source
   *          the source
  ...
  {
    String out = "AntData: [id=" + id + ", nest=" + nestName + ", team="
        + teamName + ", " + antType + ", health=" + health + ", " + myAction;
    if (carryUnits > 0)
      out += ", carry: [" + carryType + ", " + carryUnits + "]";
    if (underground)
      out += ", underground ]";
    else
      out += ", x=" + gridX + ", y=" + gridY + "]";
